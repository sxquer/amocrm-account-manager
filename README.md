...
Для такого решения нужен свой сервер на другой машине:

1. У агентов и моделей не должно быть shell, filesystem, SSH, admin/root или иного доступа к этой машине.
2. Сервер никогда и ни при каких условиях не должен отдавать наружу env-переменные, amoCRM token, process environment, secret storage, config-файлы или диагностические дампы с секретами.
3. Сервер не должен давать агенту возможность записывать или изменять env/config с amoCRM token. Иначе агент может сам записать секрет в доступный ему лог, файл, ответ API или audit payload и затем прочитать его.
4. Все write-операции должны проходить через policy enforcement на стороне remote gateway, а не через доверие к локальному MCP, prompt-инструкциям или локальным env-переменным.

В этом проекте такой remote gateway пока не реализован. Тем, кому нужна строгая security boundary, нужно реализовать и эксплуатировать такую схему самостоятельно: отдельный сервер, изолированное хранение amoCRM token, policy enforcement, audit log и отсутствие любых каналов, через которые агент может прочитать или записать секрет.
